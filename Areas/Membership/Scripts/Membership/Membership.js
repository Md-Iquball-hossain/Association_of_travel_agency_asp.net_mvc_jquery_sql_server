$(document).ready(function () {
    ...
    function MembershipVM() {
        ...
        //self.NameOfOrganization = ko.observable(); //Commented by Maruf
        self.NameOfOrganization = ko.observable('').extend({ required: true });
        self.Phone = ko.observable().extend({ required: false, pattern: { message: 'Valid Phone Number required.', params: '^(?:\\+88|01)?(?:\\d{11}|\\d{13})$' } });
        ...
        self.TINNo = ko.observable('');
        self.RepresentativeName = ko.observable();
        self.RepresentativeNID = ko.observable();
        ...
        //self.Fees = ko.observableArray([]); //Code by Maruf
        ...
        self.GetMemberList = function (searchTerm, callback) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    self.Members(data);
                    //console.log("NameofOrganization : " + ko.toJSON(self.Members(data)));
                },
                error: function () {
                    alert(error.status + "<--and--> " + error.statusText);
                }
        ...
        self.CheckExistMember = function () {
            var formData = new FormData();
            formData.append('Id', self.Id());

            formData.append('NameOfOrganization', self.NameOfOrganization());

            if (self.NameOfOrganization() != "") {
                $.ajax({
                    type: "POST",
                    url: '/Membership/Membership/CheckExistMember',
                    data: formData,
                    contentType: false,
                    processData: false,
                    cache: false,
                    success: function (data) {
                        ...
                        if (data < 0) {
                            ...
                            //$('#confirmModalText').text("Same Organization already exists ! Select Yes to proceed or No to cancel");
                            $('#confirmModalText').html("Same Organization already exists ! <br/> Select Yes to proceed or No to cancel");

                            self.GetAllDiv();
                        }
                        else {
                            $('#confirmModal').modal('show');
                            $('#confirmModalText').text("Are you sure to save all data!");
                            self.GetAllDiv();
                        }
            ...
            if (self.NameOfOrganization() == "") {
                $('#successModal').modal('show');
                $('#successModalText').text("Please enter Name of Organization");
            }
        };
        self.saveMembership = function () {
            ...
            formData.append('TIN', self.TINNo());
            ...
            //Code by Maruf
            //console.log(ko.toJSON(self.NameOfOrganization()));
            if (self.FromYear() > 0 && self.NameOfOrganization() != "") {
                $.ajax({
                    type: "POST",
                    ...
                    success: function (data) {
                        ...
                        if (data > 0) {
                            ...
                            $('#successModal').modal('show');
                            $('#successModalText').text("Same Name of Organization already exists in the system !");
                            self.GetAllDiv();
                        }
                        else if (data == 0) {
                            $('#successModal').modal('show');
                            $('#successModalText').text("Membership Application not saved.");
                            self.GetAllDiv();
                        }
                        //Need to set Issue Date to 
                    },
        ...
        //Display photo upon selection
        self.ReadURLPhoto = function readURL(input) {
            ...
            $("#representativePhoto").change(function () {
                readURL(this);
            });
        }
        ...
        self.saveContactPerson = function () {
            ...
            var photoFile = $('#representativePhoto').prop('files')[0];
            ...
            if (self.Id() > 0) {
                return $.ajax({
                    ...
                    success: function (data) {
                        ...
                        //self.IssueDate(data.IssueDate ? moment(data.IssueDate) : moment()); // Commented by Maruf
                        self.IssueDate(moment(data.IssueDate)); //Code by Maruf
                        ...
                        //Code by Maruf
                        //$.when(self.LoadFeeTypeList()).done(function () {
                        //    $.each(data.FeeTypeList, function (index, value) {
                        //        self.FeeTypeList.push(value.FeesId);
                        //    });
                        //});
                        //
                        ...
                        if (data.ContactPerson != null) {
                            ...
                            self.ContactPerson.PhotoTest(data.ContactPerson.PhotoTest);
                            self.ContactPerson.PhotoPath(data.ContactPerson.PhotoPath);
                        }
